...
import { useCommonStore, type Setting } from "@/stores/CommonStore";
...
const unsaved = storeToRefs(useCommonStore()).unsavedPreferences;
...
const save = async () => {
  waiting.value = true;
  const res = await useCommonStore().updatePreferences();
  waiting.value = false;
};
</script>
